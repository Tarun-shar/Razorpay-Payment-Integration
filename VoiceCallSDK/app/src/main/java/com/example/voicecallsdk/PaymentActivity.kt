package com.example.voicecallsdk
...
class PaymentActivity: Activity(), PaymentResultWithDataListener, ExternalWalletListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pay_btn.setOnClickListener {
            val price_get = price.text.toString()
            if(price_get.isNotEmpty()){
                startPayment(price_get)
            }
            else{
                Toast.makeText(this, "Firstly, Enter the\nPrice!", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun startPayment(price:String) {
        ...
        val price_in_subunits = price.toInt()*100
